...
function ConductorForm({ conductor, onClose }) {
  ...
  const guardar = useMutation({
    ...
    mutationFn: () => {
      ...
      return isEdit ? actualizarConductor(conductor.id, datos) : crearConductor(datos)
    },
    onSuccess: () => {
...
function AccesoApp({ conductor, cuenta, onClose }) {
  const qc = useQueryClient()
  const toast = useToast()
  ...
  const [entregado, setEntregado] = useState(null) // credenciales para mostrar una sola vez
  const [copiado, setCopiado] = useState(false)

  const guardar = useMutation({
    meta: { inline: true },
    mutationFn: () => (restablecer
      ? restablecerPassword(cuenta.id, password)
      : crearUsuarioConductor({ conductor_id: conductor.id, usuario: usuario.trim(), password })),
    onSuccess: () => {
...
export default function Conductores() {
  ...
  const [acceso, setAcceso] = useState(null) // { conductor, cuenta }
  ...
  const cuentaDe = (id) => cuentas.find((u) => u.conductor_id === id)
