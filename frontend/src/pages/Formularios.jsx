...
export default function Formularios() {
  ...
  const [modal, setModal] = useState(null)          // null | 'nuevo' | formulario
  const [modalGastos, setModalGastos] = useState(null)
  ...
  const conductoresFiltrados = useMemo(() => {
    ...
    if (!v) return activos
    const ids = [v.titular_id, v.relevador_id].filter(Boolean)
      ...
      <div className="panel p-4 mb-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr_auto] items-end">
          ...
          <div>
            ...
            <select id="f-est" className="select" value={estFilter} onChange={(e) => setEstFilter(e.target.value)}>
              ...
              <tbody>
                {visibles.map((f) => (
                  <tr key={f.id} className="group">
                    ...
                    <td className="text-ink-soft max-w-[14rem] truncate" title={f.ruta}>{f.ruta || '—'}</td>
