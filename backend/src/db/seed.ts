...
/** Filas con id: el seed solo necesita el identificador de lo que acaba de insertar. */
...
function seed(): void {
  ...
  const insert = db.transaction(() => {
    ...
    const getCuenta = (codigo: string) =>
      db.prepare('SELECT * FROM cuentas WHERE codigo = ?').get(codigo) as ConId;
    ...
    const getV = (numero: string) =>
      db.prepare('SELECT * FROM vehiculos WHERE numero_interno = ?').get(numero) as ConId;
    ...
    const getC = (nombre: string) =>
      db.prepare('SELECT * FROM conductores WHERE nombre = ?').get(nombre) as ConId;
    ...
    const getMes = (nombre: string, anio: number) =>
      db.prepare('SELECT * FROM meses WHERE nombre = ? AND anio = ?').get(nombre, anio) as ConId;
    ...
    const getF = (numero: string) =>
      db.prepare('SELECT * FROM formularios WHERE numero = ?').get(numero) as ConId;
  ...
  const count = db.prepare('SELECT COUNT(*) total FROM formularios').get() as { total: number };
