...
import { BusFront, User, Users, Plus, Pencil, Trash2 } from 'lucide-react'
...
function VehiculoForm({ vehiculo, conductores, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    titular_id: vehiculo?.titular_id ?? '',
    relevador_id: vehiculo?.relevador_id ?? '',
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  const igual = form.titular_id && form.titular_id === form.relevador_id
  ...
  const guardar = useMutation({
    ...
    mutationFn: async () => {
      ...
      const res = isEdit ? await actualizarVehiculo(vehiculo.id, datos) : await crearVehiculo(datos)
      const id = isEdit ? vehiculo.id : res.id
...
function Persona({ icono: Icono, rol, nombre }) {
  return (
