...
const { assertMesAbierto, assertFormularioEditable } = require('../db/guards');
...
router.delete('/:id', asyncHandler((req, res) => {
  ...
  const soportes = db.prepare('SELECT soporte FROM gastos WHERE formulario_id = ? AND soporte IS NOT NULL')
    .all(req.params.id).map(r => r.soporte);
  ...
  soportes.forEach(ruta => fs.unlink(path.join(RECIBOS_DIR, path.basename(ruta)), () => {}));
  res.json({ ok: true });
}));
