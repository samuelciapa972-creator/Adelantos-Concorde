...
import { ApiError, analizarSoporte, crearGastoMovil, getFormulariosMovil } from '../api'
import { comprimirImagen } from '../utils/imagen'
import { TIPOS_GASTO, ETIQUETA_TIPO, hoyISO } from '../utils/gastos'
import { useToast } from '../components/ui/Toast'
import ConfirmDialog from '../components/ui/ConfirmDialog'
import MovilLayout from './MovilLayout'
import type { AnalisisSoporte, Confianza, TipoGasto } from '../types'
...
function Lectura({ nivel }: { nivel?: Confianza | null }) {
...
interface CampoProps {
  id: string
  etiqueta: string
  nivel?: Confianza | null
  ayuda?: string
  children: ReactNode
}

function Campo({ id, etiqueta, nivel, ayuda, children }: CampoProps) {
  return (
    <div>
...
type Fase = 'foto' | 'leyendo' | 'revisar'

export default function NuevoSoporte({ onCambiarPassword }: { onCambiarPassword: () => void }) {
  const { id = '' } = useParams()
  ...
  const [local, setLocal] = useState<{ file: File; url: string | null } | null>(null)
  const [resultado, setResultado] = useState<AnalisisSoporte | null>(null)
  ...
  const [f, setF] = useState<Campos>({ valor: '', fecha: hoyISO(), tipo: 'otro', proveedor: '', nit: '', numero_documento: '', observacion: '' })
  const [conceptoManual, setConceptoManual] = useState<string | null>(null)
  const set = (k: keyof Campos) => (e: ChangeEvent<HTMLInputElement>) => setF((x) => ({ ...x, [k]: e.target.value }))
  ...
  async function procesar(file: File | undefined) {
    ...
    try {
      ...
      setErrorLectura((err as Error).message)
      setFase('foto')
    }
  }

  const onArchivo = (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; e.target.value = ''; procesar(file) }
  ...
  const guardar = useMutation({
    ...
    mutationFn: (confirmar: boolean) => {
      if (!resultado) throw new Error('Primero toma la foto del soporte')
      return crearGastoMovil({
    ...
    onError: (err) => { if (err instanceof ApiError && err.codigo === 'POSIBLE_DUPLICADO') setDuplicado(err.message) },
  })

  if (!isLoading && (!formulario || !formulario.editable)) return <Navigate to={`/f/${id}`} replace />

  const valorNum = Number(f.valor)
  const puedeGuardar = valorNum > 0 && Boolean(f.fecha) && concepto.trim().length > 0 && !guardar.isPending
  const confianza: Partial<NonNullable<AnalisisSoporte['extraido']>['confianza']> = resultado?.extraido?.confianza ?? {}
    ...
    <MovilLayout titulo="Nuevo soporte" atras={`/f/${id}`} onCambiarPassword={onCambiarPassword}>
      ...
      {fase === 'revisar' && resultado && (
        <form onSubmit={(e) => { e.preventDefault(); if (puedeGuardar) guardar.mutate(false) }} className="pb-28 space-y-5">
          ...
          {guardar.isError && !(guardar.error instanceof ApiError && guardar.error.codigo === 'POSIBLE_DUPLICADO') && (
