...
const { cerrarOcr } = await import('../src/utils/ocr.ts');
const { limpiarArchivosHuerfanos } = await import('../src/utils/archivos.ts');

// Respuestas JSON y filas de prueba: se inspeccionan campo a campo, sin tipar.
type Json = any;
type Quien = 'admin' | 'ana' | 'beto';
type Id = number | bigint;

const RECIBOS = path.join(import.meta.dirname, '../uploads/recibos');
...
let base: string;
let server: Server;
const cookies: Record<Quien, string> = { admin: '', ana: '', beto: '' };

async function api(quien: Quien, metodo: string, ruta: string, body?: unknown, extra: { raw?: string } = {}) {
  ...
  let json: Json = null; try { json = JSON.parse(t); } catch { /* respuesta no JSON */ }
  return { status: res.status, json, res, texto: t };
}

async function login(quien: Quien, usuario: string, password: string) {
  ...
  return { status: res.status, data: j.data as Json };
}
...
async function subirFoto(quien: Quien, buffer: Buffer, nombre = 'foto.jpg', tipo = 'image/jpeg') {
  const fd = new FormData();
  fd.append('foto', new Blob([new Uint8Array(buffer)], { type: tipo }), nombre);
  const res = await fetch(base + '/api/movil/analizar', { method: 'POST', headers: { cookie: cookies[quien] }, body: fd });
  return { status: res.status, json: (await res.json()) as Json };
}

let anaConductor: Id, betoConductor: Id, mesId: Id, mesCerradoId: Id, fAna: Id, fBeto: Id, fAnaCerrado: Id, fAnaLegalizado: Id;
...
test.before(async () => {
  ...
  const nf = (n: string, c: Id, mes: Id, estado = 'sin_legalizar') => db.prepare(
...
test.after(async () => {
  server.close();
  await cerrarOcr();
  for (const { nombre } of db.prepare('SELECT nombre FROM archivos').all() as { nombre: string }[]) {
    fs.rmSync(path.join(RECIBOS, nombre), { force: true });
  }
});
...
test('el administrador crea cuentas de conductor; una segunda para el mismo conductor se rechaza', async () => {
  ...
  assert.ok(lista.every((u: Json) => u.debe_cambiar_password === 1 && !('password_hash' in u)));
});
...
test('cada conductor ve solo sus formularios y gastos', async () => {
  ...
  assert.deepEqual(propios.map((f: Json) => f.numero).sort(), ['A0', 'A1', 'A2']);
  assert.equal(propios.find((f: Json) => f.numero === 'A1').editable, true);
  assert.equal(propios.find((f: Json) => f.numero === 'A0').editable, false, 'mes cerrado');
  assert.equal(propios.find((f: Json) => f.numero === 'A2').editable, false, 'ya legalizado');
  assert.equal(propios.find((f: Json) => f.numero === 'A1').saldo_real, 150000);
...
let ruta1: string, extraido1: Json;
...
test('crear gasto desde la foto: se guarda con origen app_movil y datos del soporte', async () => {
  ...
  const fila = db.prepare('SELECT origen, creado_por, nit, numero_documento FROM gastos WHERE id = ?').get(gastoId) as Json;
...
test('eliminar: solo mis gastos, con el formulario abierto; se borra también la foto', async () => {
  ...
  const archivo = (db.prepare('SELECT soporte FROM gastos WHERE id = ?').get(gastoId) as Json).soporte;
  assert.equal((await api('ana', 'DELETE', `/api/movil/gastos/${gastoId}`)).status, 200);
  await new Promise((r) => setTimeout(r, 100));
  assert.equal(fs.existsSync(path.join(RECIBOS, path.basename(archivo))), false);

  // tras legalizar, ya no se puede borrar
  const otro = (db.prepare("SELECT id FROM gastos WHERE formulario_id = ? AND origen='app_movil' LIMIT 1").get(fAna) as Json).id;
...
test('el administrador puede restablecer la contraseña y desactivar el acceso', async () => {
  const usuarioAna = (db.prepare("SELECT id FROM usuarios WHERE usuario='ana'").get() as Json).id;
  ...
  const adminId = (db.prepare("SELECT id FROM usuarios WHERE usuario='admin'").get() as Json).id;
...
test('limpieza: borra las fotos sin gasto de más de 24 h y respeta las recientes y las en uso', async () => {
  const dir = RECIBOS;
  const uid = (db.prepare("SELECT id FROM usuarios WHERE usuario='admin'").get() as Json).id;
  const crear = (nombre: string, hace: string) => {
