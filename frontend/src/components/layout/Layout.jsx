...
function ModalNuevoMes({ onClose }) {
...
function Sidebar({ onNavigate }) {
  ...
  const qc = useQueryClient()
  const toast = useToast()
  const [dialogo, setDialogo] = useState(null) // 'mes' | 'password' | 'cerrar' | 'reabrir'
  ...
  const estadoMes = useMutation({
    mutationFn: (accion) => (accion === 'cerrar' ? cerrarMes(mesId) : reabrirMes(mesId)),
    onSuccess: (_, accion) => {
