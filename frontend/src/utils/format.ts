...
export function fmtCorto(n: number | string | null | undefined): string {
  const v = Number(n) || 0
...
export const ESTADOS: Record<EstadoFormulario, { label: string; tone: Tono }> = {
  legalizado:    { label: 'Legalizado',    tone: 'ok' },
  pendiente:     { label: 'Pendiente',     tone: 'warn' },
  sin_legalizar: { label: 'Sin legalizar', tone: 'bad' },
}

export function badgeLabel(estado: string): string {
  return (ESTADOS[estado as EstadoFormulario] ?? ESTADOS.sin_legalizar).label
}
