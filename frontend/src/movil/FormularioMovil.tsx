...
import type { FormularioMovil as Formulario, GastoMovil } from '../types'

const esPdf = (ruta = ''): boolean => ruta.toLowerCase().endsWith('.pdf')
...
function Resumen({ etiqueta, valor, tono, hint, className = '' }: ResumenProps) {
  ...
  return (
    <div className={`bg-surface-sunken rounded-xl px-3.5 py-3 ${className}`}>
      ...
      <p className={`font-display text-[22px] font-semibold leading-tight whitespace-nowrap ${tono ? colores[tono] : ''}`}>{valor}</p>
      {hint && <p className="text-xs text-ink-mute mt-0.5">{hint}</p>}
    </div>
  )
}

export default function FormularioMovil({ onCambiarPassword }: { onCambiarPassword: () => void }) {
  const { id = '' } = useParams()
  const qc = useQueryClient()
  const toast = useToast()
  const [aEliminar, setAEliminar] = useState<GastoMovil | null>(null)
  ...
  const f: Formulario | undefined = formularios.find((x) => String(x.id) === id)
    ...
    <MovilLayout titulo={f ? `Formulario ${f.numero}` : 'Formulario'} atras="/" onCambiarPassword={onCambiarPassword}>
      {!f ? <Skeleton className="h-40 rounded-xl" /> : (
        <>
          ...
          {!f.editable && (
            <p className="mt-5 chip-mute !rounded-xl !px-4 !py-3 !text-sm w-full !whitespace-normal"><Lock size={14} aria-hidden="true" />{f.estado === 'legalizado' ? 'Este formulario ya fue legalizado.' : 'El mes está cerrado.'} No se pueden agregar soportes.</p>
          )}
