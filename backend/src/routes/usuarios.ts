...
import type { Conductor } from '../types/dominio.ts';
...
router.post('/', asyncHandler((req, res) => {
  ...
  const conductor = db.prepare('SELECT id, nombre, activo FROM conductores WHERE id = ?').get(conductor_id) as
    | Pick<Conductor, 'id' | 'nombre' | 'activo'>
    | undefined;
