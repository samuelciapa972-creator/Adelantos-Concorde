...
import type { TipoGasto } from '../types/dominio.ts';
...
const lineasDe = (texto: string): string[] => texto.split(/\r?\n/).map((l) => l.replace(/\s+/g, ' ').trim()).filter(Boolean);

// ── Importes ────────────────────────────────────────────────────────────────

/** "332.800" | "332,800" | "$ 332.800,00" | "1.234.567" → número en pesos, o null */
export function parseMonto(bruto: unknown): number | null {
...
const arreglarDigitos = (linea: string): string =>
...
const montosDe = (linea: string): number[] =>
  [...linea.matchAll(TOKEN_MONTO)].map((m) => parseMonto(m[0])).filter((v): v is number => v != null && v >= 500);

function extraerValor(lineas: string[]): Dato<number> | null {
  ...
  lineas.forEach((linea, i) => {
    ...
    let montos = montosDe(limpia);

    // "TOTAL" solo en la línea y la cifra en la siguiente (tiquetes en columnas)
    const siguiente = lineas[i + 1];
    if (fuerte && montos.length === 0 && siguiente) {
      montos = montosDe(arreglarDigitos(siguiente));
    }
...
function fechaValida(d: number, m: number | undefined, a: number, hoy: Date): string | null {
  if (a < 100) a += 2000;
  if (m == null || m < 1 || m > 12 || d < 1 || d > 31 || a < 2000) return null;
...
function extraerFecha(lineas: string[], hoy: Date): Dato<string> | null {
  const encontradas: { f: string; peso: number; conClave: boolean }[] = [];
...
function extraerNit(texto: string): Dato<string> | null {
...
function extraerNumeroDocumento(lineas: string[]): Dato<string> | null {
  ...
  for (const linea of lineas) {
    ...
    if (m?.[1]) return { valor: m[1].replace(/\s+/g, ' ').trim().toUpperCase(), confianza: 'media' };
  }
  return null;
}
...
const letras = (s: string): number => (s.match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/g) ?? []).length;

function extraerProveedor(lineas: string[]): Dato<string> | null {
  const cabecera = lineas.slice(0, 8);
  const limpia = (l: string): string => l.replace(/\bnit\b.*$/i, '').replace(/[|_~*=]+/g, ' ').replace(/^[^A-Za-zÁÉÍÓÚÑ0-9]+|[^A-Za-zÁÉÍÓÚÑ0-9.)]+$/g, '').replace(/\s+/g, ' ').trim();
...
const REGLAS_TIPO: [TipoGasto, RegExp][] = [
...
function clasificarTipo(texto: string): Dato<TipoGasto> {
...
/**
 * @param texto  salida del OCR
 * @param hoy    para validar fechas (se inyecta en las pruebas)
 */
export function extraerDatos(texto: string | null | undefined, hoy = new Date()): DatosExtraidos {
