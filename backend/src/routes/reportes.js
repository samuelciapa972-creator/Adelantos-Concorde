...
const { PNG_PATH } = require('../utils/convertLogo')
const fs           = require('fs')
const { SALDOS_COLUMNS } = require('../db/saldos')
...
const nombreSeguro = (s) => String(s ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\w.-]+/g, '_')

const getFormularios = (mesId, vehiculoId) => {
...
const getMes = (mesId) =>
  db.prepare('SELECT * FROM meses WHERE id = ?').get(mesId)
...
router.get('/excel/:mesId', asyncHandler(async (req, res) => {
  ...
  const vehiculoInfo = vehiculo_id
    ? db.prepare('SELECT numero_interno FROM vehiculos WHERE id = ?').get(vehiculo_id)
    : null
  ...
  const totValues = COLS.map(c => {
    ...
    return rows.reduce((s, r) => s + (r[c.key] ?? 0), 0)
  })
...
router.get('/pdf/:mesId', (req, res) => {
  ...
  rows.forEach((r, i) => {
    ...
    valores.forEach((val, idx) => {
      doc.text(String(val), x + 2, y + 4, { width: pdfCols[idx].width - 4, lineBreak: false, align: 'center' })
      x += pdfCols[idx].width
    })
  ...
  totales.forEach((val, idx) => {
    doc.text(val, x + 2, y + 5, { width: pdfCols[idx].width - 4, lineBreak: false, align: 'center' })
    x += pdfCols[idx].width
  })
  y += 20
