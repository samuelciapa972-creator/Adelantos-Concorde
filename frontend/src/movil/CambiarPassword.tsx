...
interface Props {
  /** Pantalla completa sin salida: primer ingreso con la contraseña temporal. */
  obligatorio?: boolean
  onListo?: () => void
  onCancelar?: () => void
}

export default function CambiarPassword({ obligatorio = false, onListo, onCancelar }: Props) {
  ...
  const formulario = (
    <form onSubmit={(e) => { e.preventDefault(); if (!distinta) cambiar.mutate() }} className="space-y-4">
      ...
      <button type="submit" className="btn-primary btn-lg w-full" disabled={cambiar.isPending || distinta}>{cambiar.isPending ? 'Guardando…' : 'Guardar contraseña'}</button>
