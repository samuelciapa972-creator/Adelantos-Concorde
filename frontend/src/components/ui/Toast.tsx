...
type ToastApi = Record<TipoToast, (mensaje: string) => void>

const ToastContext = createContext<ToastApi | null>(null)

const ICONOS: Record<TipoToast, LucideIcon> = { ok: CheckCircle2, error: CircleAlert, info: Info }
const TONOS: Record<TipoToast, string> = {
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const quitar = useCallback((tid: number) => setToasts((t) => t.filter((x) => x.id !== tid)), [])
  ...
  // Errores globales de mutaciones (ver main.tsx)
  useEffect(() => {
    const onError = (e: Event) => mostrar((e as CustomEvent<string>).detail || 'Ocurrió un error', 'error')
