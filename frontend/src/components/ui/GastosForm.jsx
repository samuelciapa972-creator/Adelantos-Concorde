...
const VACIO = { fecha: '', tipo: 'combustible', concepto: '', valor: '', observacion: '', proveedor: '', nit: '', numero_documento: '' }

function Resumen({ etiqueta, valor, hint, tono = 'neutral' }) {
  const colores = { neutral: 'bg-surface-sunken text-ink', ok: 'bg-ok-soft text-ok', bad: 'bg-bad-soft text-bad' }
  return (
...
export default function GastosForm({ formulario, readOnly = false }) {
              ...
              <tbody>
                {gastos.map((g) => (
                  <tr key={g.id}>
                    ...
                    <td><span className="chip-mute">{ETIQUETA[g.tipo] ?? g.tipo}</span></td>
        ...
        <section className="border border-line rounded-xl p-4" aria-label="Agregar gasto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); agregar.mutate() }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Tipo" required>
              {(id) => <select id={id} className="select" value={form.tipo} onChange={set('tipo')}>{TIPOS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select>}
            </Field>
