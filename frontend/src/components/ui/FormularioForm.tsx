import { useMemo, useState, type ChangeEvent, type FormEvent, type InputHTMLAttributes, type ReactNode } from 'react'
...
import type { EstadoFormulario, Formulario, FormularioInput } from '../../types'
...
/** Los campos se editan como texto (así llegan de los inputs) y se convierten al enviar. */
interface Campos {
  numero: string
  fecha_dia: string
  conductor_id: string
...
const texto = (v: string | number | null | undefined): string => (v == null ? '' : String(v))
...
export default function FormularioForm({ formulario, onClose }: Props) {
  ...
  const [form, setForm] = useState<Campos>({
    ...
    fecha_dia: texto(formulario?.fecha_dia),
    conductor_id: texto(formulario?.conductor_id),
    vehiculo_id: texto(formulario?.vehiculo_id ?? vehiculoId),
    mes_id: texto(formulario?.mes_id ?? mesId),
    fecha_salida: formulario?.fecha_salida ?? '',
    fecha_regreso: formulario?.fecha_regreso ?? '',
    anticipo: texto(formulario?.anticipo),
    tasa_uso: texto(formulario?.tasa_uso),
    hospedaje: texto(formulario?.hospedaje),
    mantenimiento: texto(formulario?.mantenimiento),
  ...
  const set = (campo: keyof Campos) => (e: ChangeEvent<Control>) => setForm((f) => ({ ...f, [campo]: e.target.value }))
  ...
  const opcionesConductor = useMemo(() => {
    ...
    const ids = vehiculo ? [vehiculo.titular_id, vehiculo.relevador_id].filter((x): x is number => x != null) : []
    return ids.length ? activos.filter((c) => ids.includes(c.id) || c.id === formulario?.conductor_id) : activos
  }, [conductores, vehiculo, formulario])

  function cambiarVehiculo(e: ChangeEvent<HTMLSelectElement>) {
    ...
    setForm((f) => ({
      ...
      conductor_id: v && ![v.titular_id, v.relevador_id].includes(Number(f.conductor_id)) ? texto(v.titular_id) : f.conductor_id,
    }))
  }

  const mutation = useMutation({
    mutationFn: (data: FormularioInput) => (formulario ? actualizarFormulario(formulario.id, data) : crearFormulario(data)),
    meta: { inline: true },
    onSuccess: () => {
  ...
  const dinero: InputHTMLAttributes<HTMLInputElement> = { type: 'number', min: 0, step: 1, inputMode: 'numeric', className: 'input tabular-nums' }
