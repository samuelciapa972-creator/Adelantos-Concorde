...
const claveIntento = (req: Request, usuario: string) => `${req.ip}|${String(usuario).toLowerCase()}`;
...
const totalUsuarios = () => (db.prepare('SELECT COUNT(*) AS n FROM usuarios').get() as Conteo).n;
...
function esPeticionLocal(req: Request): boolean {
...
router.post('/setup', asyncHandler((req, res) => {
  ...
  res.status(201).json({ ok: true, data: auth.publico({ id: Number(id), usuario, nombre, rol: 'admin' }) });
}));
...
router.post('/login', asyncHandler((req, res) => {
  ...
  const u = db.prepare('SELECT * FROM usuarios WHERE usuario = ? AND activo = 1').get(usuario) as UsuarioFila | undefined;
...
router.post('/password', asyncHandler((req, res) => {
  ...
  const u = db.prepare('SELECT * FROM usuarios WHERE id = ?').get(sesion.id) as UsuarioFila;
