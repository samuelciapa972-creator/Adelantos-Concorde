...
function cuentaParaVehiculo(numeroInterno, cuentaId) {
  if (cuentaId) {
    if (!db.prepare('SELECT 1 FROM cuentas WHERE id = ?').get(cuentaId)) {
      const err = new Error('La cuenta indicada no existe'); err.status = 400; throw err
    }
    return cuentaId
  }
  const existente = db.prepare('SELECT id FROM cuentas WHERE codigo = ?').get(numeroInterno)
