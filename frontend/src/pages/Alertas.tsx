...
import { CircleAlert, CheckCircle2, Clock3, TriangleAlert, CalendarDays, Lock, type LucideIcon } from 'lucide-react'
...
interface AccionesFila {
  onLegalizar: (id: number) => void
  deshabilitado: boolean
  ocupado: boolean
}

function FilaAlerta({ f, onLegalizar, deshabilitado, ocupado }: AccionesFila & { f: Formulario }) {
...
interface GrupoProps extends AccionesFila {
  titulo: string
  icono: LucideIcon
  tono: 'bad' | 'warn'
...
function Grupo({ titulo, icono: Icono, tono, formularios, ...props }: GrupoProps) {
...
export default function Alertas() {
  ...
  const props: AccionesFila = { onLegalizar: (id) => legalizar.mutate(id), deshabilitado: cerrado, ocupado: legalizar.isPending }
