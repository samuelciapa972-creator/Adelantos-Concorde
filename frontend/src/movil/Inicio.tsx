...
import type { FormularioMovil } from '../types'

function TarjetaFormulario({ f }: { f: FormularioMovil }) {
...
export default function Inicio({ onCambiarPassword }: { onCambiarPassword: () => void }) {
  ...
  // Agrupados por mes
  const grupos: { clave: string; cerrado: boolean; items: FormularioMovil[] }[] = []
  formularios.forEach((f) => {
    const clave = `${f.mes_nombre} ${f.mes_anio}`
    const g = grupos.find((x) => x.clave === clave)
    if (g) g.items.push(f)
    else grupos.push({ clave, cerrado: Boolean(f.mes_cerrado), items: [f] })
  })
  ...
  return (
    <MovilLayout titulo="Mis formularios" onCambiarPassword={onCambiarPassword}>
      <section aria-label="Resumen" className="mb-5">
        ...
        {perfil && perfil.vehiculos.length > 0 && (
